...
export default function FavoritesPage({ products, onBack, onOpenProduct, onToggleFavorite }: FavoritesPageProps) {
  return (
    <main className="store-favorites-page">
      ...
      <header className="store-favorites-heading"><div><span className="store-kicker">Your saved collection</span><h1>Things you&apos;d like<br /><em>to keep close.</em></h1></div><p>Save a fish, plant or habitat while you decide. Your favorites are saved securely to your account.</p></header>
      {products.length ? <div className="store-favorites-grid">{products.map((product) => <article className="store-favorite-card" key={product.id} role="button" aria-label={`View ${product.name}`} tabIndex={0} onClick={() => onOpenProduct(product)} onKeyDown={(event) => { if (event.target !== event.currentTarget) return; if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpenProduct(product); } }}>
