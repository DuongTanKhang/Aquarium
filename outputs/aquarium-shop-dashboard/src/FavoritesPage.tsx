...
export default function FavoritesPage({ products, onBack, onOpenProduct, onToggleFavorite }: FavoritesPageProps) {
  return (
    <main className="store-favorites-page">
      ...
      {products.length ? <div className="store-favorites-grid">{products.map((product) => <article className="store-favorite-card" key={product.id}>
        <button type="button" className="store-favorite-card-image" onClick={() => onOpenProduct(product)}><img src={productImage(product)} alt={product.name} /><span>{product.inStock ? "In the water" : "Resting"}</span></button>
        <div><span className="store-kicker">{product.category.name}</span><h2>{product.name}</h2><strong>{price(product.price)}</strong><button type="button" className="store-text-link" onClick={() => onToggleFavorite(product.id)}>Remove <span>×</span></button></div>
