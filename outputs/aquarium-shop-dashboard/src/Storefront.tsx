...
export default function Storefront() {
  ...
  const favoriteStorageKey = `aquarium-store-favorites:${customerUser?.id ?? "guest"}`;
  const [favoriteIds, setFavoriteIds] = useState<string[]>(() => typeof window !== "undefined" ? loadFavoriteIds("aquarium-store-favorites:guest") : []);
  ...
  return (
    <div className="storefront">
      ...
      {customerAuthOpen && <CustomerAuthModal onAuthenticated={handleCustomerAuthenticated} onClose={() => { setCustomerAuthOpen(false); setPendingAddProduct(null); setPendingAddQuantity(1); }} />}
