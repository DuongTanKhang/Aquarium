...
export default function Storefront() {
  ...
  const openFavoriteProduct = (product: PublicProduct) => {
    // Close the favorites route before opening the shared detail view. Keeping
    // this transition in one handler avoids a stale favorites page swallowing
    // the click when React batches the two state updates.
    window.history.pushState({}, "", "/shop");
    setFavoritesOpen(false);
    setSelectedProductImage(0);
    setDetailQuantity(1);
