...
export default function ReturnsWorkspace({ onSessionExpired }: { onSessionExpired: () => void }) {
  ...
  const load = async () => {
    ...
    try { setRows(await listAdminReturnRequests()); } catch (requestError) { setError(displayError(requestError)); if (requestError instanceof ApiError && requestError.status === 401) { clearAccessToken(); onSessionExpired(); } } finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  const changeStatus = async (row: ReturnRequest, status: string) => {
    if (status === row.status) return;
    const providerRefundId = status === "REFUNDED" ? window.prompt("Enter the processor refund reference before confirming:", row.providerRefundId ?? "")?.trim() : undefined;
    if (status === "REFUNDED" && !providerRefundId) return;
    setUpdating(row.id); setError(""); setNotice("");
    try { const updated = await updateAdminReturnRequest(row.id, { status: status as ReturnRequestStatus, providerRefundId }); setRows((current) => current.map((item) => item.id === row.id ? updated : item)); setNotice(`${row.orderNumber} moved to ${labels[status]}.`); } catch (requestError) { setError(displayError(requestError)); } finally { setUpdating(""); }
  };
