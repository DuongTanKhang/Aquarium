...
const DEFAULT_METHODS: PaymentMethodConfig[] = [
  { id: "CARD", provider: "CARD", label: "Credit & debit cards", description: "Visa, Mastercard, American Express and Discover.", enabled: true, setupNote: "Card details are handled by the secure checkout processor; never stored in this dashboard." },
...
export class PaymentsService {
   ...
   */
  async createPayPalCheckout(dto: CreateOrderDto, customerId: string): Promise<PayPalCheckoutStartResponse> {
    const configured = await this.getPayPalCheckoutConfig();
    const order = await this.orders.createPublic({ ...dto, paymentMethod: "PAYPAL" }, customerId);
    try {
      ...
      const response = await fetch(`${configured.apiBase}/v2/checkout/orders`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
      ...
      await this.prisma.payment.update({ where: { orderId: order.id }, data: { providerPayload: payload as Prisma.InputJsonValue } });
  ...
  async capturePayPalCheckout(orderId: string, customerId: string): Promise<PayPalCheckoutCaptureResponse> {
    ...
    const accessToken = await this.getPayPalAccessToken(configured.apiBase, configured.clientId, configured.clientSecret);
    const response = await fetch(`${configured.apiBase}/v2/checkout/orders/${encodeURIComponent(provider.paypalOrderId)}/capture`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
    ...
    const row = await this.prisma.$transaction(async (transaction) => {
      ...
      await transaction.payment.update({ where: { orderId }, data: { status: "PAID", transactionCode: captureId, paidAt: new Date(), providerPayload: nextPayload as Prisma.InputJsonValue } });
  ...
  async receivePayPalWebhook(headers: Record<string, string | string[] | undefined>, body: Record<string, unknown>): Promise<{ received: true }> {
    ...
    const configured = await this.getPayPalCheckoutConfig();
    const accessToken = await this.getPayPalAccessToken(configured.apiBase, configured.clientId, configured.clientSecret);
    const verifyResponse = await fetch(`${configured.apiBase}/v1/notifications/verify-webhook-signature`, {
      method: "POST",
  ...
  private async getPayPalAccessToken(apiBase: string, clientId: string, clientSecret: string): Promise<string> {
    ...
    const response = await fetch(`${apiBase}/v1/oauth2/token`, { method: "POST", headers: { Authorization: `Basic ${basic}`, "Content-Type": "application/x-www-form-urlencoded" }, body: "grant_type=client_credentials" });
  ...
  private async verifyPayPalMerchant(merchantId: string): Promise<void> {
    ...
    const response = await fetch(`${apiBase}/v1/customer/partners/${encodeURIComponent(partnerMerchantId)}/merchant-integrations/${encodeURIComponent(merchantId)}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  async getSettings(): Promise<PaymentSettingsResponse> {
    ...
    if (!stored) return { country: "US", currency: "USD", defaultMethod: "CARD", methods: DEFAULT_METHODS, updatedAt: new Date() };
  ...
  private serialize(row: { country: string; currency: string; defaultMethod: string; methods: unknown; updatedAt: Date }): PaymentSettingsResponse {
    ...
    const methods = DEFAULT_METHODS.map((method) => ({ ...method, enabled: storedMethods.find((item) => item.id === method.id)?.enabled ?? method.enabled }));
    const defaultMethod = PAYMENT_METHOD_IDS.includes(row.defaultMethod as PaymentMethodId) ? row.defaultMethod as PaymentMethodId : "CARD";
