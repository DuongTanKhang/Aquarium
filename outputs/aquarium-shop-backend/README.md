...
- `POST /api/v1/payments/paypal/orders/:id/resume` — safely creates a new PayPal approval session for an existing pending order (without duplicating the order or stock reservation).
...
- Cash on delivery confirms the order immediately with an `UNPAID` payment until the delivery is collected. Card checkout still needs a PCI-compliant hosted card processor before it can be enabled for real charges; raw card data is never accepted.
